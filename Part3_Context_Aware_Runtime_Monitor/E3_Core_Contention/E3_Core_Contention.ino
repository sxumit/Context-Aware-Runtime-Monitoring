...
// IMPORTANT:
//
// This function is NOT called for every sample.
//
// It is called only once every
// ADAPTIVE_CHECK_INTERVAL samples.
//
// A mode change requires
// MODE_HYSTERESIS_COUNT consecutive
// confirmations.
//
// =====================================================
...
void updateAdaptiveMode(
  ...
  // Only evaluate every 10 samples
  if (
...
// =====================================================
// DETERMINE WHETHER DETAILED MONITORING IS NEEDED
// =====================================================
//
// Basic monitoring:
//
...
// - raw timing
//
// happens for EVERY sample.
//
// Detailed calculations are reduced under
// heavier CPU pressure.
//
// FULL     -> every sample
// BALANCED -> every 2nd sample
// LIGHT    -> every 4th sample
// CRITICAL -> every 4th sample
//
// =====================================================
...
void monitoredTask(
  ...
  double overheadPercentage =
    (averageMonitoringOverhead / averageWorkloadTime) * 100.0;
  ...
  double taskCpuUtilization =
    (sumWorkloadTime / experimentTimeUs) * 100.0;


  double monitorCpuUtilization =
    (monitorBusyTimeUs / experimentTimeUs) * 100.0;
  ...
  double monitoringCoverage =
    ((double)monitorEventsProcessed / (NUM_SAMPLES * 2.0)) * 100.0;
  ...
  double deadlineAnomalyRate =
    ((double)anomaliesDetected / NUM_SAMPLES) * 100.0;
  ...
  Serial.println(
    "Part 3 Experiment 3 adaptive run complete.");
...
void contenderTask(
  ...
  // Small delay so normal task can start
  vTaskDelay(
    pdMS_TO_TICKS(
      1000));
  ...
  while (true) {
    ...
    // -----------------------------------------------
    // IMPORTANT:
    // Give Core 0 scheduling time back
    // -----------------------------------------------
